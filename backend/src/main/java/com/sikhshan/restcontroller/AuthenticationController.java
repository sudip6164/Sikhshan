...
public class AuthenticationController {
	...
	public ResponseEntity<?> loginSuperadmin(@RequestBody LoginRequest loginRequest) {
	    ...
	    if (optionalUser.isPresent()) {
	        ...
	        if (user.getRole().name().equalsIgnoreCase("student") || user.getRole().name().equalsIgnoreCase("faculty")) {
	            return ResponseEntity.status(403).body("Access denied: Students and faculty are not allowed to log in here");
	        }
